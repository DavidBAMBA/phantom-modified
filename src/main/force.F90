...
!
   ...
   character(len=80), parameter, public :: &  ! module version
      modid="$Id: ba795aac7d1c34d6dbd6856154a16dbcf68c7aaa $"
...
subroutine compute_forces(i,iamgasi,iamdusti,xpartveci,hi,hi1,hi21,hi41,gradhi,gradsofti, &
   ...
   real    :: gradpj,pro2j,projsxj,projsyj,projszj,sxxj,sxyj,sxzj,syyj,syzj,szzj,dBrhoterm, dBrhoterm2
      ...
      is_sph_neighbour: if (q2i < radkern2 .or. q2j < radkern2) then
         ...
         if (iamgasi .and. iamgasj) then
            ...
            if (mhd) then
               ...
               divBdiffterm = -pmassj*projdB*(grkerni + grkernj)*0.5*rho1j    !Deleted and add the average kernel
...
!------------------
            ...
            if (ien_type == ien_etotal) then ! total energy
               dudtdissi = - pmassj*((pro2i*rhoi*rho1j + qrho2i)*projvj*(grkerni + grkernj)*0.5 + &
                                    (pro2j*rhoj*rho1i + qrho2j)*projvi*(grkerni + grkernj)*0.5)
            else
...
!------------------
            ...
            !--add av term to pressure               Deleted
            !gradpi = pmassj*(pro2i + qrho2i)*grkerni
            gradpi = pmassj*(pro2i*rhoi*rho1j + qrho2i)*(grkerni + grkernj)*0.5 

            !if (usej) gradpj = pmassj*(pro2j + qrho2j)*grkernj                  Deleted
            if (usej) gradpj = pmassj*(pro2j*rhoj*rho1i + qrho2j)*(grkerni + grkernj)*0.5  
            ...
            if (mhd) then
               ...
               dBdissterm = (avBterm*grkerni + avBtermj*grkernj)*vsigB     !termino disipativo pendiente
               !dBdissterm = (avBterm*rhoi*rho1j + avBtermj*rhoj*rho1i)*vsigB*(grkerni + grkernj)*0.5


               !--energy dissipation due to artificial resistivity   
               if (useresistiveheat) dudtresist = -0.5*dB2*dBdissterm

               !pmjrho21grkerni = pmassj*rho21i*grkerni             DELETED
               pmjrho21grkerni = pmassj*rho1j*rho1i*(grkerni + grkernj)*0.5

               !pmjrho21grkernj = pmassj*rho21j*grkernj               DELETE
               pmjrho21grkernj = pmassj*rho1j*rho1i*(grkerni + grkernj)*0.5


               termi        = pmjrho21grkerni*projBi

               divBsymmterm =  termi + pmjrho21grkernj*projBj  !term of momentum

               dBrhoterm    = -termi 
               !dBrhoterm2   = pmjrho21grkerni*projv
               dBrhoterm2 = pmassj*rho1j*0.5*(grkerni + grkernj)
               !
               ! grad psi term for divergence cleaning
               ! cleaning evolving d/dt (psi/c_h) as in Tricco, Price & Bate (2016)
               !
               !dpsiterm = overcleanfac*(pmjrho21grkerni*psii*vwavei + pmjrho21grkernj*psij*vwavej) DELETED
            ...
            !projsyi = (sxyi*runix + syyi*runiy + syzi*runiz)*grkerni   Deleted Grad kernel, averaged grad kern applied
            !projszi = (sxzi*runix + syzi*runiy + szzi*runiz)*grkerni
   
            projsxi = (sxxi*runix + sxyi*runiy + sxzi*runiz)*(grkerni + grkernj)*0.5*rho1i*rho1j*rhoi*rhoi
            projsyi = (sxyi*runix + syyi*runiy + syzi*runiz)*(grkerni + grkernj)*0.5*rho1i*rho1j*rhoi*rhoi
            projszi = (sxzi*runix + syzi*runiy + szzi*runiz)*(grkerni + grkernj)*0.5*rho1i*rho1j*rhoi*rhoi
   
            if (usej) then
               !projsxj = (sxxj*runix + sxyj*runiy + sxzj*runiz)*grkernj
               !projsyj = (sxyj*runix + syyj*runiy + syzj*runiz)*grkernj   Deleted Grad kernel, averaged grad kern applied
               !projszj = (sxzj*runix + syzj*runiy + szzj*runiz)*grkernj
               projsxj = (sxxj*runix + sxyj*runiy + sxzj*runiz)*(grkerni  + grkernj)*0.5*rho1i*rho1j*rhoj*rhoj
               projsyj = (sxyj*runix + syyj*runiy + syzj*runiz)*(grkerni  + grkernj)*0.5*rho1i*rho1j*rhoj*rhoj
               projszj = (sxzj*runix + syzj*runiy + szzj*runiz)*(grkerni  + grkernj)*0.5*rho1i*rho1j*rhoj*rhoj
            ...
            !fsum(idrhodti) = fsum(idrhodti) + projv*grkerni             !deleted REVISAR
            fsum(idrhodti) = fsum(idrhodti) + projv*(grkerni + grkernj)*0.5*rho1j*rhoi         
...
!
   ...
   !
      ...
      syyi  = syyi - pmassi*Brhoyi*Brhoyi        !Delete rho square term in stress tensor
...
!
   ...
   loop_over_neighbours2: do n = 1,nneigh
      ...
      is_sph_neighbour: if (q2i < radkern2 .or. q2j < radkern2) then
         ...
         if (q2i < radkern2) then
            grkerni = grkern(q2i,qi)*hfacgrkern
            !grkerni = grkern(q2i,qi)                  !Deleted Omega term 
         ...
         if (iamgasi .and. iamgasj) then
            ...
            if (mhd) then
               ...
               dB2 = dBx*dBx + dBy*dBy + dBz*dBz
               !divBdiffterm = -pmassj*projdB*grkerni    
               divBdiffterm = -pmassj*projdB*(grkerni+grkernj)/hfacgrkern    !Deleted and add the average kernel
            endif
         else
            !-- v_sig for pairs of particles that are not gas-gas
            vsigi = max(-projv,0.0)
...
!------------------
            ...
            if (mhd) then
               ...
               !pmjrho21grkerni = pmassj*rho21i*grkerni                    Deleted term rho squared in B evolution and apply average gradient
               pmjrho21grkerni = pmassj*rho1i*rho1j*(grkerni/hfacgrkern + grkernj/hfacgrkern)
               
               !pmjrho21grkernj = pmassj*rho21j*grkernj                    Deleted term rho squared in and add average kernel in psi term evolution equation for B
               pmjrho21grkernj = pmassj*rho1j*rho1i*(grkerni/hfacgrkern + grkernj/hfacgrkern)

               termi        = pmjrho21grkerni*projBi
               divBsymmterm =   termi+ pmjrho21grkernj*projBj             ! f_divB term in momentum equation tensile instability correction
               dBrhoterm    = -termi
               !
               ! grad psi term for divergence cleaning
               ! cleaning evolving d/dt (psi/c_h) as in Tricco, Price & Bate (2016)
            ...
            projsxi = (sxxi*runix + sxyi*runiy + sxzi*runiz)*(grkerni + grkernj)*rho1i*rho1j/hfacgrkern
            projsyi = (sxyi*runix + syyi*runiy + syzi*runiz)*(grkerni + grkernj)*rho1i*rho1j/hfacgrkern
            projszi = (sxzi*runix + syzi*runiy + szzi*runiz)*(grkerni + grkernj)*rho1i*rho1j/hfacgrkern

            if (usej) then
               !projsxj = (sxxj*runix + sxyj*runiy + sxzj*runiz)*grkernj
               !projsyj = (sxyj*runix + syyj*runiy + syzj*runiz)*grkernj   Deleted Grad kernel, averaged grad kern applied
               !projszj = (sxzj*runix + syzj*runiy + szzj*runiz)*grkernj
               projsxj = (sxxj*runix + sxyj*runiy + sxzj*runiz)*(grkerni  + grkernj)*rho1i*rho1j/hfacgrkern
               projsyj = (sxyj*runix + syyj*runiy + syzj*runiz)*(grkerni  + grkernj)*rho1i*rho1j/hfacgrkern
               projszj = (sxzj*runix + syzj*runiy + szzj*runiz)*(grkerni  + grkernj)*rho1i*rho1j/hfacgrkern
            ...
            !--calculate divv for use in du, h prediction, av switch etc.
            !fsum(idrhodti) = fsum(idrhodti) + projv*grkerni       ! Deleted and add average grad kernel in divV term in cleanong ecuation phi

            fsum(idrhodti) = fsum(idrhodti) + projv*(grkerni+grkernj)/hfacgrkern*rho1i         
...
!
      !Brhoxi = Bxi*rho1i
      !Brhoyi = Byi*rho1i        Delete rho square term in stress tensor
      !Brhozi = Bzi*rho1i
      Brhoxi = Bxi
      Brhoyi = Byi
      Brhozi = Bzi
...
!--construct total isotropic pressure term (gas + magnetic + stress)
!
      !pro2i = (pri + radPi)*rho1i*rho1i + stressiso + 0.5*Bro2i     Deleted rho square term in pressure term in stress tensor
      pro2i = (pri + radPi) + stressiso*rhoi*rhoi + 0.5*Bro2i      
...
!--construct m*p/(rho^2 \Omega) in force equation using pressure
!
      !pro2i  = (pri + radPi)*rho1i*rho1i + stressiso      Deleted rho square term in pressure
      pro2i = (pri + radPi) + stressiso*rhoi*rhoi
      vwavei = spsoundi
   endif
...
#endif
<<<<<<< HEAD
               if (lightcurve) then divvi0i*(fsum(idendtdissi)+fsum(idudtdissi))
=======
               if (lightcurve) then
...
>>>>>>> bb654c34b755fa9698556d3593fbe0482b742aba
         ...
         if (mhd) then
            ...
            !
<<<<<<< HEAD
            ! hyperbolic/parabolic cleaning terms (dpsi/dt) from Tricco &  ce (2012)
=======
...
<<<<<<< HEAD
               ...
               dBevol(4,i) = -vcleani*fsum(idivBdiffi) - psii*dtau - 0.5*psii*divvi       !Deleted rho1i in cleaning equation

=======
               !dBevol(4,i) = -vcleani*fsum(idivBdiffi)*rhoi - psii*dtau - 0.5*psii*divvi      

               dBevol(4,i) = -vcleani*fsum(idivBdiffi) - psii*dtau - 0.5*psii*divvi*rhoi       !Deleted rho1i in cleaning equation
>>>>>>> bb654c34b755fa9698556d3593fbe0482b742aba
