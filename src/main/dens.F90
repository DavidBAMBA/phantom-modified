...
module densityforce
   ...
   !
    ...
    character(len=80), parameter, public :: &  ! module version
       modid="$Id: bf666407fade3914043fc8b3bcd634ed04ef74c3 $"
    ...
    integer, parameter :: &
          ...
          isum_f_den       = irhodustiend + 4, &
          isum_f_num       = irhodustiend + 5
   ...
   !
       if (q2i < radkern2) then
          ...
          sametype: if (same_type) then
            ...
            !------------------------------------------------------------------------
            !                Add terms to the sums for f term
            !------------------------------------------------------------------------
             rhoi = rhoh(real(hi),  massoftype(igas))
             rhoj = rhoh(xyzh(4,j), massoftype(igas))
             term_num = (massoftype(iphasej)/rhoi)*rij2*grkerni
             term_den = (massoftype(iphasej)/rhoj)*rij2*grkerni
             rhosum(isum_f_num) = rhosum(isum_f_num) + term_num
             rhosum(isum_f_den) = rhosum(isum_f_den) + term_den
    ...
    !----------------------------------------------------------------------
    !                Calculate fi term to correction in gradient
    !----------------------------------------------------------------------
    if (rhosum(isum_f_den) > 0.0) then
      fi = 0.5 * (rhosum(isum_f_num) / rhosum(isum_f_den) )
      grkerni = grkerni * fi
    endif
   !----------------------------------------------------------------------
   !----------------------------------------------------------------------
